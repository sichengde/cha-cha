...
CREATE TABLE IF NOT EXISTS query_pages (
  ...
  settings JSON COMMENT '查询设置(JSON格式)',
  ...
  query_time_limit TIMESTAMP NULL COMMENT '查询时间限制，NULL表示不限制',
...
CREATE TABLE IF NOT EXISTS query_records (
  ...
  user_id VARCHAR(36) COMMENT '查询用户ID',
  openid VARCHAR(100) COMMENT '查询用户openid',
  nickname VARCHAR(100) COMMENT '查询用户昵称',
...
CREATE TABLE IF NOT EXISTS signatures (
  ...
  user_id VARCHAR(36) COMMENT '签收用户ID',
  openid VARCHAR(100) COMMENT '签收用户openid',
  nickname VARCHAR(100) COMMENT '签收用户昵称',

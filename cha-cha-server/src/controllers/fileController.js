...
const exportSignatures = async (req, res) => {
  try {
    const { id } = req.params
    const userId = req.user.id
    ...
    const signatures = await query(
      `SELECT s.*, qd.row_index 
       FROM signatures s 
       JOIN query_data qd ON s.query_data_id = qd.id 
       WHERE s.query_page_id = ?`,
      [id]
    )
    ...
    dataRows.forEach(row => {
      const data = JSON.parse(row.data)
      const rowData = headers.map(h => data[h.column_index] || '')
      const sign = signMap[row.id]
      if (sign) {
        rowData.push(
          sign.sign_type === 'signature' ? '手写签名' : '按钮确认',
          sign.signed_at
        )
        sheetData.push(rowData)
      }
    ...
    const worksheet = XLSX.utils.aoa_to_sheet(sheetData)
    XLSX.utils.book_append_sheet(workbook, worksheet, '签收记录')

    const fileName = `${queryPage.name}_签收记录_${Date.now()}.xlsx`
    const exportPath = path.join(uploadDir, fileName)
    XLSX.writeFile(workbook, exportPath)
